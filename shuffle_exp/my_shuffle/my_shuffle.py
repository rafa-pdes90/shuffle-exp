...
def weighted_shuffle(x, y, w):
  ...
  if len(songs_list) > 1:
    ...
    for songs in songs_list:
      ...
      while i < j:
        first = songs[i][1]/(song_count-1)
        last = songs[j][1]/(song_count-1)
        sub_count = j-i + 1
        ...
        lim_min = songs_pos[songs[i][0]] = first
        lim_max = songs_pos[songs[j][0]] = last
        i += 1
        j -= 1
      
      if i == j:
        songs_pos[songs[i][0]] = songs[i][1]/(song_count-1)
